      ...
      <div class="content">
        <div class="top">
          <div class="search">
            ...
            <button v-show="!notFind && user.name !== null" class="btn" @click="addFriend">
              加入好友
            </button>
...
export default {
  ...
  methods: {
    search() {
      this.axios.get('/SearchUser', { params: { account: this.account } }).then((res) => {
        this.newFriesn = null;
        this.account = null;
        if (!res.data.success) {
          this.notFind = true;
    ...
    addFriend() {
      this.axios.post('/AddFriend', { account: this.user.account }).then((res) => {
        // 需要判斷重複好友
        if (res.data.success) {
